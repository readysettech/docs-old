...
<div class="option-details" markdown="1">
For a [distributed ReadySet deployment](../../guides/production-notes.md#scale-out), store cached query results with the Adapter instead of the Server.
...
<div class="option-details" markdown="1">
For a [distributed ReadySet deployment](../../guides/production-notes.md#scale-out) with multiple ReadySet Adapters, each Adapter needs to know about the caches on the ReadySet Server. This flag sets the interval, in seconds, at which to poll the ReadySet Server.
...
These examples focus on running a [standard ReadySet deployment](../../guides/production-notes.md#standard) (i.e., ReadySet Server and Adapter running as a single process on a single machine). For running a distributed ReadySet deployment, see the [`readyset-server` examples](readyset-server.md#examples).
